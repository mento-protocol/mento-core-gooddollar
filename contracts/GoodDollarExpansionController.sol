    ...
    OwnableUpgradeable
{
    ...
    function getExpansionConfig(
        bytes32 exchangeId
    ) public view returns (ExchangeExpansionConfig memory) {
        require(
            exchangeExpansionConfigs[exchangeId].expansionRate > 0,
            "Expansion config not set"
        );
    ...
    function setGoodDollarExchangeProvider(
        ...
        require(
            _goodDollarExchangeProvider != address(0),
            "GoodDollarExchangeProvider address must be set"
        );
        goodDollarExchangeProvider = IGoodDollarExchangeProvider(
            _goodDollarExchangeProvider
        );
     ...
     * @param _distributionHelper The address of the distribution helper contract.
     */
    function setDistributionHelper(
        address _distributionHelper
    ) public onlyOwner {
        require(
            _distributionHelper != address(0),
            "DistributionHelper address must be set"
        );
    ...
    function mintUBIFromInterest(
        ...
        IBancorExchangeProvider.PoolExchange
            memory exchange = IBancorExchangeProvider(
                address(goodDollarExchangeProvider)
            ).getPoolExchange(exchangeId);

        uint256 amountToMint = goodDollarExchangeProvider.mintFromInterest(
            exchangeId,
            reserveInterest
        );

        require(
            IERC20(exchange.reserveAsset).transferFrom(
                msg.sender,
                reserve,
                reserveInterest
            ),
        ...
        IGoodDollar(exchange.tokenAddress).mint(
    ...
    function mintUBIFromReserveBalance(
        ...
        uint256 contractReserveBalance = IERC20(exchange.reserveAsset)
            .balanceOf(reserve);
        uint256 additionalReserveBalance = contractReserveBalance -
            exchange.reserveBalance;
        if (additionalReserveBalance > 0) {
            amountMinted = goodDollarExchangeProvider.mintFromInterest(
                exchangeId,
    ...
    function mintUBIFromExpansion(
        ...
        bool shouldExpand = block.timestamp >
            config.lastExpansion + config.expansionFrequency;
        if (shouldExpand || config.lastExpansion == 0) {
            ...
            if (config.lastExpansion == 0) {
                ...
                numberOfExpansions =
                    ...
                    config.expansionFrequency;
            }

            uint256 stepExpansionScaler = MAX_WEIGHT - config.expansionRate;
            uint256 expansionScaler = unwrap(
                powu(wrap(stepExpansionScaler), numberOfExpansions)
            );

            exchangeExpansionConfigs[exchangeId].lastExpansion = uint32(
                block.timestamp
            );
            amountMinted = goodDollarExchangeProvider.mintFromExpansion(
                exchangeId,
                expansionScaler
            );
