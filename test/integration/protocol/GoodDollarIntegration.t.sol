...
import {IGoodDollar} from "contracts/interfaces/IGoodProtocol.sol";
...
contract GoodDollarIntegrationTest is Test {
    ...
    address public constant GoodDollarAvatar =
    ...
    function configureReserve() public {
        ...
        initialAssetAllocationWeights[0] = FixidityLib
            ...
            .unwrap();
        initialAssetAllocationWeights[1] = FixidityLib
            .newFixedFraction(1, 2)
            .unwrap();

        uint256 tobinTax = FixidityLib.newFixedFraction(5, 1000).unwrap();
        uint256 tobinTaxReserveRatio = FixidityLib
            .newFixedFraction(2, 1)
            .unwrap();
        ...
        require(
            reserve.isStableAsset(address(gdToken)),
            "GoodDollar is not a stable token in the reserve"
        );
        require(
            reserve.isCollateralAsset(address(reserveToken)),
    ...
    function configureTokens() public {
        ...
        require(
            gdToken.isMinter(address(expansionController)),
            "ExpansionController is not a minter"
        );
    ...
    function configureBroker() public {
        ...
        require(
            broker.isExchangeProvider(address(exchangeProvider)),
            "ExchangeProvider is not registered in the broker"
        );
        require(
            broker.exchangeReserve(address(exchangeProvider)) ==
                address(reserve),
    ...
    function configureExchangeProvider() public {
        ...
        exchangeId = IBancorExchangeProvider(address(exchangeProvider))
            .createExchange(poolExchange1);
    }
    ...
    function test_SwapIn_reserveTokenToGDollar() public {
        ...
        uint256 priceBefore = IBancorExchangeProvider(address(exchangeProvider))
            .currentPrice(exchangeId);
        ...
        uint256 priceAfter = IBancorExchangeProvider(address(exchangeProvider))
            .currentPrice(exchangeId);
    ...
    function test_expansion() public {
        uint256 priceBefore = IBancorExchangeProvider(address(exchangeProvider))
            .currentPrice(exchangeId);
        uint256 distributionHelperBalanceBefore = gdToken.balanceOf(
            distributionHelper
        );
        ...
        assertEq(
            gdToken.balanceOf(distributionHelper),
    ...
    function test_interest() public {
        ...
        uint256 priceAfter = IBancorExchangeProvider(address(exchangeProvider))
            .currentPrice(exchangeId);
        uint256 reserveBalanceAfter = reserveToken.balanceOf(address(reserve));
        uint256 interestCollectorBalanceAfter = reserveToken.balanceOf(
            reserveInterestCollector
        );
        uint256 distributionHelperBalanceAfter = gdToken.balanceOf(
            distributionHelper
        );

        assertEq(reserveBalanceAfter, reserveBalanceBefore + reserveInterest);
        assertEq(
            interestCollectorBalanceAfter,
            interestCollectorBalanceBefore - reserveInterest
        );
        assertTrue(
            distributionHelperBalanceBefore < distributionHelperBalanceAfter
        );
        assertEq(priceBefore, priceAfter);
    }
}
